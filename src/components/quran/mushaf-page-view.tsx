...
export function MushafPageView({ surahNumber, initialVerseNumber, onBack }: MushafPageViewProps) {
    ...
    useEffect(() => {
        if (initialVerseNumber && pageData) {
            const found = pageData.ayahs.find(
                a => a.surah.number === surahNumber && a.numberInSurah === initialVerseNumber
            );
            if (found) {
    ...
    const handleCopyVerse = useCallback((ayah: PageAyah) => {
        const text = `${stripTajweed(ayah.text)} (${isArabic ? ayah.surah.name : ayah.surah.englishName}:${ayah.numberInSurah})`;
        navigator.clipboard.writeText(text);
        toast({ title: isArabic ? 'تم نسخ الآية' : 'Verse copied' });
        setSelectedAyah(null);
    }, [isArabic, toast]);

    const handlePlayVerse = useCallback((ayah: PageAyah) => {
        const verse = {
    ...
    const { hizbNumber, quarterLabel } = getHizbInfo(hizbQuarter, isArabic ? 'ar' : 'en');
    ...
    return (
        <div className="mushaf-container flex flex-col" onClick={() => setSelectedAyah(null)}>
            ...
            <div className="fixed top-0 left-0 right-0 z-30 flex items-center gap-2 px-2 py-1.5 border-b border-border/40 bg-background/95 backdrop-blur-md">
                ...
                <div className="flex items-center gap-1 text-[10px] text-muted-foreground shrink-0">
                    ...
                    <span>{isArabic ? `ح${hizbNumber}` : `H${hizbNumber}`}{quarterLabel}</span>
                    <span className="opacity-40">•</span>
                    <span className="font-mono">{currentPage}</span>
                </div>

                <button
                    onClick={(e) => { e.stopPropagation(); handlePlayPage(); }}
                    className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-foreground/10 transition-colors shrink-0"
                >
                    {isPagePlaying
            ...
            {/* Tajweed legend */}
            {isTajweed && <div className="shrink-0"><TajweedLegend /></div>}
...
const MushafPageContent = React.memo(function MushafPageContent({
    ...
    useEffect(() => {
        const adjustScale = () => {
            ...
            if (contentHeight > containerHeight) {
                const newScale = Math.max(0.4, containerHeight / contentHeight);
                setScale(newScale);
            } else {
                setScale(1);
            }
    ...
    const surahGroups: { surahNumber: number; surahName: string; ayahs: typeof page.ayahs; isNewSurah: boolean }[] = [];
    ...
    return (
        <div
            ref={contentRef}
            className="mushaf-page font-quran text-[1.25rem] leading-[2.3] text-right w-full h-full"
            dir="rtl"
        ...
        >
            {surahGroups.map((group, gi) => (
                <div key={`${group.surahNumber}-${gi}`}>
                    ...
                    {/* Bismillah */}
                    {group.isNewSurah && group.surahNumber !== 1 && group.surahNumber !== 9 && (
                    ...
                    {group.ayahs.map((ayah) => {
                        ...
                        return (
                            <span
                                key={ayah.number}
                                className={`inline relative ${isHighlighted ? 'mushaf-highlight' : ''} ${isSelected ? 'bg-primary/15 rounded' : ''} ${isActiveVerse ? 'mushaf-playing' : ''}`}
